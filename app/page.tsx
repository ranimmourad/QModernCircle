...
export default function HomePage() {
  ...
  useEffect(() => {
    fetch('/api/menu', { cache: 'no-store' })
      ...
      .then(data => {
        if (data.items) {
          // Translate Supabase columns to match our design
          setDrinks(data.items.map((item: any) => ({
            name: item.name,
            price: `${item.price} TND`,
            img: item.image,       // Supabase uses 'image'
            desc: item.description  // Supabase uses 'description'
          })));
        }
      }).catch(console.error);
    ...
    <>
      ...
      <section id="about" className="py-32 px-6 md:px-10">
        <div className="max-w-6xl mx-auto grid md:grid-cols-2 gap-16 md:gap-24 items-center">
          <FadeUp delay={0.2}>
            ...
            <h2 className="font-display text-5xl md:text-6xl tracking-wider leading-tight text-cocoa">A quieter<br />way to gather.</h2>
            <p className="font-heading italic text-2xl text-bark mt-6">Coffee, craft, and conversation — without the noise.</p>
            <p className="mt-8 text-bark leading-loose">Modern Circle began as a small idea: a calm, non-smoking space in El Mourouj where mornings feel unhurried, work feels possible, and every object on the shelf has been chosen with care. Six years on, it remains exactly that — a sanctuary for slow days and handmade things.</p>
            <div className="grid grid-cols-3 gap-6 mt-12 pt-12 border-t border-cocoa/10">
              <div><p className="font-display text-4xl md:text-5xl text-cocoa"><AnimatedCounter to={6} suffix="+" /></p><p className="text-xs uppercase tracking-widest text-bark mt-2">Years Open</p></div>
              <div><p className="font-display text-4xl md:text-5xl text-cocoa"><AnimatedCounter to={32} /></p><p className="text-xs uppercase tracking-widest text-bark mt-2">Artisans</p></div>
              <div><p className="font-display text-4xl md:text-5xl text-cocoa"><AnimatedCounter to={100} suffix="%" /></p><p className="text-xs uppercase tracking-widest text-bark mt-2">Handmade</p></div>
            </div>
          </FadeUp>
          <FadeUp delay={0.4}>
      ...
      <section className="py-32 px-6 md:px-10 bg-cream">
        <div className="max-w-7xl mx-auto">
          ...
          <FadeUp delay={0.1}>
            <Swiper modules={[Autoplay, Pagination, Navigation]} slidesPerView={1.2} spaceBetween={24} centeredSlides={false} autoplay={{ delay: 3000, disableOnInteraction: false }} pagination={{ clickable: true }} navigation={true} breakpoints={{ 640: { slidesPerView: 2.2, spaceBetween: 24 }, 1024: { slidesPerView: 3.2, spaceBetween: 32 } }} className="pb-16">
              {drinks.map((d) => (
      ...
      <section className="relative h-[70vh] overflow-hidden bg-[#3E2723]">
        ...
        <div className="relative z-10 h-full flex items-center justify-center px-6">
          <FadeUp className="text-center max-w-3xl">
            ...
            <h2 className="font-display text-5xl md:text-7xl tracking-wider text-cream leading-tight">Bring your laptop.<br /><span className="italic-accent text-terracotta">Stay a while.</span></h2>
      ...
      <section className="py-32 px-6 md:px-10 bg-cream">
        <div className="max-w-4xl mx-auto">
          ...
          <FadeUp delay={0.1}>
            <Swiper modules={[Autoplay, Pagination, EffectFade]} effect="fade" fadeEffect={{ crossFade: true }} autoplay={{ delay: 6000, disableOnInteraction: false }} pagination={{ clickable: true }} loop className="pb-16">
              {reviews.map((r) => (
                <SwiperSlide key={r.id || r.name}>
        ...
        <div className="max-w-3xl mx-auto">
          ...
          <FadeUp delay={0.1}>
            <form onSubmit={(e) => { e.preventDefault(); alert("Thank you. We will confirm your booking shortly."); }} className="grid md:grid-cols-2 gap-8">
              <div><label className="block text-xs uppercase tracking-widest text-cream/60 mb-3">Date</label><input type="date" required className="w-full bg-transparent border-b border-cream/30 py-3 text-cream focus:border-terracotta focus:outline-none transition-colors" /></div>
              <div><label className="block text-xs uppercase tracking-widest text-cream/60 mb-3">Time</label><input type="time" required className="w-full bg-transparent border-b border-cream/30 py-3 text-cream focus:border-terracotta focus:outline-none transition-colors" /></div>
              <div><label className="block text-xs uppercase tracking-widest text-cream/60 mb-3">Guests</label><select required className="w-full bg-transparent border-b border-cream/30 py-3 text-cream focus:border-terracotta focus:outline-none transition-colors"><option className="text-[#3E2723]">1 guest</option><option className="text-[#3E2723]">2 guests</option><option className="text-[#3E2723]">3 guests</option><option className="text-[#3E2723]">4 guests</option><option className="text-[#3E2723]">5 guests</option><option className="text-[#3E2723]">6+ guests</option></select></div>
              <div><label className="block text-xs uppercase tracking-widest text-cream/60 mb-3">Phone</label><input type="tel" required placeholder="+216 ..." className="w-full bg-transparent border-b border-cream/30 py-3 text-cream placeholder-cream/30 focus:border-terracotta focus:outline-none transition-colors" /></div>
              <div className="md:col-span-2"><label className="block text-xs uppercase tracking-widest text-cream/60 mb-3">Special Requests</label><textarea rows={3} placeholder="Birthday, dietary notes, seat by the window..." className="w-full bg-transparent border-b border-cream/30 py-3 text-cream placeholder-cream/30 focus:border-terracotta focus:outline-none transition-colors resize-none" /></div>
              <div className="md:col-span-2 mt-6"><button type="submit" className="w-full md:w-auto px-12 py-4 border border-cream text-cream text-xs uppercase tracking-widest hover:bg-cream hover:text-cocoa transition-colors duration-500">Request Reservation</button></div>
            </form>
          </FadeUp>
        </div>
      </section>
          ...
          <div className="grid md:grid-cols-2 gap-12">
            <FadeUp>
              <div className="border border-cocoa/15 aspect-[4/3] overflow-hidden">
                <iframe src="https://www.google.com/maps?q=El+Mourouj+6+Tunisia&output=embed" width="100%" height="100%" style={{ border: 0, filter: "grayscale(0.4)" }} loading="lazy" referrerPolicy="no-referrer-when-downgrade" />
              </div>
            </FadeUp>
            <FadeUp delay={0.1}>
              <div>
                ...
                <div className="space-y-4 text-bark">
                  <p><span className="text-xs uppercase tracking-widest block mb-1">Address</span>Av. Martyrs, Résidence Nermine<br />El Mourouj 6, Tunisia</p>
                  <p><span className="text-xs uppercase tracking-widest block mb-1">Phone</span>+216 29 379 400</p>
                  <p><span className="text-xs uppercase tracking-widest block mb-1">Hours</span>Monday — Sunday<br />08:00 — 23:00</p>
                </div>
                    ...
                    {faqs.map((f, i) => (
                      <div key={i} className="border-b border-cocoa/10 pb-3">
                        <button onClick={() => setOpenFaq(openFaq === i ? null : i)} className="w-full text-left flex items-center justify-between py-2 text-cocoa">
                          ...
                          {openFaq === i ? <Minus size={16} strokeWidth={1.25} className="flex-shrink-0" /> : <Plus size={16} strokeWidth={1.25} className="flex-shrink-0" />}
                        </button>
                        <AnimatePresence>
                          {openFaq === i && (
